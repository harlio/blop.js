    ...
    Plugin.prototype = {
        ...
        bindUIActions: function() {
            ...
            $(s.elem).on('click', s.opt.blk + s.opt.elClrBtn, function(e) {
                e.preventDefault();
                $(s.elem).find(s.opt.blk + s.opt.elInput).attr('value', '');
                s.showDropzone();
        ...
        replaceImg: function(img, callback) {
            ...
            if (!(img.src || img instanceof HTMLCanvasElement)) {
                ...
                content = $('<a target="_blank">').append(img)
                    .attr('download', s.opt.currentFile.name)
                    .attr('href', img.src || img.toDataURL(s.opt.currentFile.type));
            }
        ...
        handleCrop: function() {
            ...
            var imgNode = $(s.elem).find(s.opt.blk + s.opt.elPrevImg),
                img = imgNode[0];
            ...
            imgNode.Jcrop({
                ...
                boxWidth:       imgNode.width(),
                boxHeight:      imgNode.height(),
